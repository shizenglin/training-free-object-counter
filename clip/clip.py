...
__all__ = ["available_models", "load", "tokenize", "encode_text_with_prompt_ensemble","encode_text_with_prompt",
...
def encode_text_with_prompt(model, text, device, prompt_templates=None):

    # using default prompt templates for ImageNet
    if prompt_templates == None:
        prompt_templates = ['a photo of many {}.','a photo of the small {}']

    prompted_t = [template.format(text) for template in prompt_templates]
    prompted_t = tokenize(prompted_t).to(device)
    class_embeddings = model.encode_text(prompted_t)
    class_embeddings /= class_embeddings.norm(dim=-1, keepdim=True)
    class_embedding = class_embeddings.mean(dim=0)
    ...
    text_features = class_embedding.to(device).t().unsqueeze(0)
...
# sm shape N_t
def similarity_map_to_points(sm, shape, t=0.9, points_num=1, down_sample=2):
