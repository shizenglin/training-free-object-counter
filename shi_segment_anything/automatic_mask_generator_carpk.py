...
class SamAutomaticMaskGenerator:
    ...
    @torch.no_grad()
    def generate(self, image: np.ndarray,ref_infor) -> List[Dict[str, Any]]:
        """
        ...
        # Generate masks
        mask_data, mask_size = self._generate_masks(image,ref_infor)
    ...
    def _generate_similarity(self, image, ref_infor):
        target_feat = ref_infor[0]
        target_embedding = ref_infor[1]
        mask_size = ref_infor[2]
    ...
    def _generate_masks(self, image: np.ndarray,ref_infor) -> MaskData:
        orig_size = image.shape[:2]
        self.predictor.set_image(image)
        sim, target_embedding, mask_size = self._generate_similarity(image, ref_infor)
